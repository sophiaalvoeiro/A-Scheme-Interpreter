...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        frame_news = Frame.make_child_frame(self.env, self.formals ,args)
        return frame_news
        # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    forms = nil
    while bindings != nil:
        binding_now = bindings.first
        check_form(binding_now, 2, 2)
        forms = Pair(binding_now.first, forms)
        valp1 = scheme_eval(binding_now.second.first, env)
        vals = Pair(valp1, vals)
        bindings = bindings.second
    check_formals(forms)
    return env.make_child_frame(forms, vals)
    # END PROBLEM 15
